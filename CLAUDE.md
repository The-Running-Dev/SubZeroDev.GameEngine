...
- `docs/docusaurus.config.ts`, `docs/sidebar.ts` — **local overrides** of the base image's
  defaults (autogenerated sidebar; broken-link checks set to `throw`, so a bad link fails
  the build rather than warning past review). Spec docs reference `src/engine/` code by
  relative path — that code is in this repo (`../../src/engine/…`).
