...
- `docs/docusaurus.config.ts`, `docs/sidebar.ts` — **local overrides** of the base image's
  defaults (autogenerated sidebar; both Docusaurus broken-link checks are `'warn'` by
  design — the hard gate is `build/Test-Documentation.ps1`, which fails the build on a
  relative link or anchor; see `agent.md`, *Two link checks*). Spec docs reference
  `src/engine/` code by relative path — that code is in this repo (`../../src/engine/…`).
- `docs/static/index.html` — a meta-refresh forwarding the bare domain to `/docs/`. Files
  under `static/` are copied verbatim into the build root, so this is what makes
  `https://game-engine.subzerodev.com/` serve instead of 404.
