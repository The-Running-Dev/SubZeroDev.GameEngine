...
### 2026-08-04 — Kit upgrade to `9896915`: kept the manual-gate `/slice`/`/track` workflow, added the context-warning hook and the measurement fixes
Context: The kit advanced five commits past the recorded `9b8313c` install. The largest change (`92abff1`) automates `/slice` to branch, commit, push, open a draft PR, and tick `Done when` boxes itself, with `AGENTS.md`'s *Tracking work* changed to match. This repository's `slice.md`/`track.md`/`kit-help.md`/`pr.md` already state, deliberately, that `/slice` stops after reporting criteria by id and that ticking is the user's confirmation, not the agent's — a human gate this repository chose on purpose, not a staleness gap. The remaining four commits are a `UserPromptSubmit` context-size warning hook, `Measure-Session.ps1` defaulting to JSON output with `-Human` for the table, and a vendor-detection guard that refuses foreign transcripts instead of reporting zero.
Chosen: Keep this repository's manual-gate workflow untouched in `slice.md`, `track.md`, `kit-help.md`'s workflow section, and `AGENTS.md`'s *Tracking work* carve-outs — per the kit's own reconciliation rule, the target's stated rule wins over the kit's newer default. Installed everything else cleanly: the `UserPromptSubmit` hook in `.claude/settings.json` (alongside the existing `SessionEnd` hook, neither one previously present for that event); `tools/Measure-Session.ps1` and its new `tools/Measure-Session.Tests.ps1` replaced wholesale (the prior copy carried no local edits — pure version lag, confirmed by diffing against the `9b8313c` baseline); `.github/workflows/verify.yml` added (Pester CI for `tools/*.ps1` — nothing in this repository's existing `ci.yml`/`docs-ci.yml`/`docs-deploy.yml`/`release-engine-package.yml` covered it); `install-all.md`'s hook-wording updated from "the `SessionEnd` hook" to "the `SessionEnd` and `UserPromptSubmit` hooks" in both places, since `install-all.md` documents which hooks any unattended install must skip and this repository now has both; two small additive `CLAUDE.md` edits — the fresh-session banner format (*Session boundaries*) and the Codex/Copilot measurement caveat (*Budget discipline*), both stated in the kit's `AGENTS.md` and absent here, neither conflicting with anything this repository customized.

Diffing the other ten "Repository overlay" commands (`contract.md`, `design.md`, `kit-help.md`, `make-human-docs.md`, `reconcile.md`, `redteam.md`, `refine.md`, `slices.md`, `pr.md`) against the kit's current HEAD found each identical to the kit's body apart from its own overlay paragraph or `W`-id rewrite — nothing in the kit's five-commit gap touched their non-overlay text, so none needed a merge. `resolve.md` and (for the one line the diff first flagged) `install-all.md`'s `$kitRoot` git-root resolution were found to be **ahead** of the kit, not behind — both are local fixes already covered by this repository's own prior decision-log entries — and were left untouched.
Rejected: **Adopt the kit's automated `/slice` workflow** — rejected; it would remove a human confirmation gate this repository's own commands say was deliberate, for less manual work per unit, and would need coordinated edits across four command files plus `CLAUDE.md`. **Skip the `UserPromptSubmit` hook** — rejected; `pwsh` 7 is already confirmed on `PATH` (the working `SessionEnd` hook proves it), the hook always exits 0, and it directly serves the *Session boundaries* rule already in `CLAUDE.md`. **Leave `Measure-Session.ps1` on the old version to avoid churn** — rejected; no local edits existed to lose, so this was the plain "re-install advances an unedited file" case, not a fork.
Reversibility: cheap — every change here is independently revertible; nothing destructive, nothing staged or committed.
