...
### 2026-08-31 — W100 contract gate 1: weekly tuning lands as flat, optional `SimulationCampaign` fields, not a new `ScenarioDefinition` block
Context: `design/30-slices.md` W100 is contract-gated on "a fresh `/contract` pass that fixes the tuning fields and defaults, the `plan_empty` policy, relationship transition, attendance formula, and the specific wisdom consumer" — the empty-plan gate was itself deferred at W50.4 for exactly this reason ("`SimulationCampaign` declares no toggle to condition a disablement branch on… the natural home is a new `SimulationCampaign`/`ScenarioDefinition` field, decided against that concrete need," recorded above). `SimulationCampaign` (`src/engine/src/kinds/simulation/campaign.ts`) is the campaign-root envelope carrying one scenario (`scenarioId`) plus flat fields (`goalFailurePrecedence`, `startingEffects`) `20-contract.md` had never pointed at.
Chosen: `emptyPlanPolicy?: "permit" | "forbid"`, `relationshipDrift?: readonly RelationshipDriftRule[]`, and `attendanceTracking?: AttendanceTrackingConfig` land flat on `SimulationCampaign`, stated in `20-contract.md` new §7.11, all three absent by default. Flat, not nested under `ScenarioDefinition` (§7.8): a campaign here plays exactly one scenario, so the two locations would name the same value, and the flat placement matches `goalFailurePrecedence`'s own existing precedent rather than inventing a second convention for the same kind of field.
Rejected: **Nest all three under `ScenarioDefinition`** — defensible as "these are scenario rules," but `ScenarioDefinition` is otherwise pure starting-condition/goal content (§7.8), and every other campaign-wide toggle this kind has (`goalFailurePrecedence`) already sits flat on the envelope; splitting the convention across two fields authored the same way, in the same campaign, for no behavioural reason was rejected. **One combined `weeklyRules` object** wrapping all three — rejected; the three tuning values have no shared validation or lifecycle (one gates a reason code, one drives a weekly system, one drives another), and grouping them would make a campaign that wants only `emptyPlanPolicy` carry two unrelated optional sub-fields it doesn't use, for no reader benefit over three top-level optionals.
Reversibility: cheap. All three fields are additive and optional; a campaign predating them is unaffected, and removing an unused field before any campaign adopts it costs nothing.

### 2026-08-31 — W100 contract gate 2: `relationships`' weekly drift is authored rules, applied to every actor, emitted only on change
Context: Recorded above (2026-08-05 entry) — `relationships` is the kind's last end-of-week stub, explicitly left there because "no weekly relationship rule — decay, drift, or otherwise — is specified anywhere in `10-simulation-kind.md`," with the instruction "revisit when a weekly relationship rule is actually specified; writing one is `/contract` work, not a slice's."
Chosen: `RelationshipDriftRule` (§7.11): an optional `categories` filter over `RelationshipState.category` (§6.11) plus four optional per-dimension integer deltas (`affinityDelta`/`trustDelta`/`respectDelta`/`resentmentDelta`), applied in array order and clamped to 0–100 per §6.2's declared range. The system runs over every actor's relationships — the player's and each `WorldState.agents[].actor`'s alike (§7.10) — not the player only, matching §6.2's "one shape, one code path" rule for actor-state systems generally. A changed dimension emits one visible `StateChange`, `reason: "relationship_drift"`; an unchanged one (already clamped, or the rule named no delta for it) emits nothing — the same `if (after === before) continue` rule `needs`' own drift already uses (`endOfWeek.ts`), applied here rather than invented fresh.
Rejected: **A single global decay rate with no per-category or per-dimension control** — simpler, but "drift" upstream and in this kind's own design intent is not uniform: a `"professional"` relationship and a `"personal"` one plausibly decay differently, and collapsing that at the contract layer would foreclose it structurally rather than leave it to campaign content, which is where every other piece of balance material in this kind already lives (§2.5, §6.5, §6.9). **Player-only drift, deferring rivals** — rejected; §7.10 already established "the player and every rival share one shape" specifically so a system need not be re-derived when a rival first exists, and `WorldState.agents` being empty today costs nothing to include now versus revisiting later.
Reversibility: cheap. `relationshipDrift` is additive and optional; every shipped campaign omits it, so the `relationships` system stays the no-op it has always been until a campaign opts in.

### 2026-08-31 — W100 contract gate 3: employment attendance is a rolling average over `workedThisWeek`, opt-in per campaign
Context: Recorded above (2026-08-08 entry) — `Employment.attendanceRatio` is declared "0–100, rolling" but maintained by nothing; the entry named the missing mechanism decision as "what a missed work week *is*… a mechanism decision belonging to whichever unit first needs attendance to matter."
Chosen: `AttendanceTrackingConfig { windowWeeks: number }` (§7.11), optional on `SimulationCampaign`. When present, the `employment` end-of-week system (§3) — which already resets `player.flags.workedThisWeek` each week — recomputes `attendanceRatio` as a rolling average using that existing flag as its sole planned/worked input: "planned" is having `currentEmployment` at all, "worked" is `workedThisWeek`. `attendanceRatio = clamp(round(((attendanceRatio × (windowWeeks − 1)) + (workedThisWeek ? 100 : 0)) / windowWeeks), 0, 100)`, using `Math.round` — the same rounding rule `education`'s course-side attendance ratio already uses, extended rather than reinvented. A changed ratio emits `reason: "attendance_updated"`, visible, on the existing emit-only-on-change convention.
Rejected: **A scheduled-shift model** (a declared expected `work` count per week, compared against actual) — the more literally "planned vs. worked" reading of W100.4's wording, and rejected because this kind has no concept of a scheduled shift anywhere in state or content (the 2026-08-08 entry names this explicitly as the missing piece), so adding one is a materially larger surface — a new content field, new validation, a new relationship to `JobDefinition` — for a distinction (`workedThisWeek` already is a boolean planned/worked signal at weekly grain) the shipped mechanism does not yet need. **Always-on tracking with no config** — rejected outright; it would move `attendanceRatio` away from its current constant `100` for every 0.10 campaign the moment this contract lands, breaking W100.7's byte-identical requirement for a field nothing has ever read.
Reversibility: cheap. `attendanceTracking` is additive and optional; every shipped campaign omits it, so `attendanceRatio` stays exactly the hire-time constant it has always been until a campaign opts in.

### 2026-08-31 — W100 contract gate 4: wisdom's consumer is the existing generic `Requirement`/`Condition` path, not a new mechanism
Context: Issue [#275](https://github.com/The-Running-Dev/SubZeroDev.GameEngine/issues/275) and `20-contract.md` §6.6 record "`wisdom` has no consumer specified anywhere in this contract or upstream." W100.5 asks that wisdom gain "the contract-selected visible projection and at least one typed content read" proving two actors with different wisdom receive different available behaviour.
Chosen: No new type. `SimulationView.attributes` (§9) is `Omit<AttributeState, "luck">` — wisdom has always had a visible projection, so the open item was never about visibility. `Requirement.type: "attribute"` (§8.1) already gates on any `AttributeState` key through a `Condition` (04 §18) over `player.attributes.wisdom`, one of §6.1's declared `DerivedPath`s, exactly as it does for every other attribute today. Wisdom's consumer is therefore any campaign `Requirement` (on an `EventChoice`, `OpportunityDefinition`, `JobDefinition`, or similar) that targets that path — an authoring fact, not a contract gap. Authoring the actual fixture Requirement that exercises it is W100.5's own `/slice` work.
Rejected: **A dedicated wisdom-specific mechanism** (a `wisdom`-only requirement type, or a new condition operator) — rejected against §8's own stated bar: this kind adds no condition operators "unless a concrete campaign need justifies each individually," and no such need exists here that the generic `attribute` path does not already serve identically to every other attribute. Inventing one now would special-case wisdom for no mechanical reason, the opposite of the "closed the gap without opening a new one" thing §8.1 already gets right for the other six.
Reversibility: cheap — a decision that a gap is already closed, not a new field. Nothing in the tree changes as a result of this gate alone.

### 2026-08-31 — W101 contract gate 1: rivals are `ScenarioDefinition.rivals: RivalConfig[]`, closing §7.10's own named gap
Context: `design/30-slices.md` W101 is contract-gated on "a fresh `/contract` pass that declares project/business definitions, their durable progress, scenario rival selection, and deterministic competition. The four action names and engine-owned strategy seam alone are not sufficient public contracts." §7.10 already named its own resolution: "the natural home is a new `ScenarioDefinition` field (e.g. `rivals: Array<{ strategyId: string }>`), decided against a concrete need rather than guessed at here."
Chosen: `RivalConfig { agentId, strategyId, displayNameKey, startingBackgroundId, initialConditions? }` on `ScenarioDefinition.rivals?: readonly RivalConfig[]` (§7.8). `initialState` builds one `AgentState` per entry, in array order, seeding `actor: ActorState` from `startingBackgroundId`'s `BackgroundDefinition` (§7.9) — the same mechanism the player's own starting state already uses — with `initialConditions` applied on top, then `DifficultyDefinition.rivalStartingAdvantages` (already specified) outermost. `strategyId` names a registered `AgentStrategy` (§7.10); an unresolved one is `unknown_rival_strategy` (§10, §14), a new Tier 1 code. Absent or empty `rivals` builds `WorldState.agents: []`, exactly today's behaviour (W101.4).
Rejected: **A separate `RivalDefinition` content type**, referenced by id from `ScenarioDefinition` the way `GoalDefinition` is — rejected because nothing about a rival's configuration is reused across scenarios the way a goal or job can be; a rival is scenario-specific by nature (this scenario's story needs this rival), so the indirection would add a lookup with no reuse to justify it. **Reusing `BackgroundDefinition` alone, with no `initialConditions`** — rejected against W101.4's own "initial conditions" wording, which implies a scenario can hand a rival something beyond a stock background (a head start, a specific disadvantage) without authoring a whole new `BackgroundDefinition` for one rival in one scenario.
Reversibility: cheap. Additive and optional; every shipped scenario omits `rivals` and is unaffected.

### 2026-08-31 — W101 contract gate 2: contested resources resolve through one pure, score-and-id comparator, never a random draw or claim order
Context: W101.7 requires that "two actors contesting a finite opportunity or job opening are resolved by the contract's complete comparator" and that the result not depend on claim order; W101.6 requires that "reordering registry construction or enabling a recording emitter does not change choices, allocation, state, or outcome." §2.2 already scarcity-models `JobOpening`/`PromotionPath`/`OpportunityDefinition` but had never specified how a tie between simultaneous claimants resolves — the existing "Just filled" paragraph explicitly deferred this: "Until rivals exist, the only filling this engine can observe is the player's own hire… it widens to rivals without changing shape."
Chosen: `resolveContest(claims: ContestClaim[], positionsAvailable: number)` (§2.2), a pure function over `{ actorId, score }` claims: sorts by `(score desc, actorId asc)`, takes the first `positionsAvailable` as `won`, the rest as `lost`. No RNG inside the comparator itself — a resolver may draw on `ctx.rng` while *computing* a claim's `score`, but the tie-break itself is a total order over already-computed values, so replay never depends on which claim was constructed first. A `lost` claim is `Revoked` (§2.3's existing table) or the job/promotion equivalent; only `won` claims apply completion rewards.
Rejected: **Resolve by construction/array order** (first claim wins) — rejected outright; it is exactly the "iteration order is not part of the state contract" failure this kind's own sorted-iteration rule (§2) exists to prevent, and would make the player's own advantage or disadvantage against a rival depend on an implementation detail (which loop ran first) rather than a stated rule. **A random draw among tied claims** — considered, since a coin-flip tie-break is a common game-design choice; rejected because `score` is already the resolver's own comparison surface (a job application's qualification, computed however that resolver likes, RNG included), so a *second*, comparator-level random draw on top would let two identical-`score` claims flip a result the resolvers' own logic already treated as equal, adding nondeterminism-shaped surface (a stream `resolveContest` would have to own) for a case the resolvers can already avoid by not producing exact ties when they don't want them.
Reversibility: moderate. `resolveContest` is new, used only where `AgentState`s exist to contest anything (today: nowhere, until a campaign declares `rivals`), so no shipped campaign's replay changes. Changing the tie-break rule later would change outcomes for any campaign that has since shipped with rivals contesting ties.

### 2026-08-31 — W101 contract gate 3: business cashflow is a new `business` end-of-week system, not folded into `finance_income`
Context: W101.3 requires business "weekly revenue, expenses" to "post once in the ordered weekly pipeline." §3's end-of-week order is normative and covered by test (W96); inserting a new system name changes that ordering rather than extending an existing one.
Chosen: A new `business` system, positioned immediately after `finance_income` and before `inventory`/`housing` (§3). Revenue and expenses post together, once per `"operating"` `BusinessRecord`, using the same `multiply`-then-round-once modifier composition §7.1 already specifies for every other modified figure in this kind. A `BusinessRecord` whose `cashOnHandCents` drops below `minimumCashCents` after that post closes immediately (`closedReason: "business_insolvent"`) — no separate grace-period ladder distinct from `HousingState.evictionStage` (§6.9), since nothing in W101's `Done when` asks for one.
Rejected: **Fold business cashflow into `finance_income`** — rejected because `finance_income` is specified, both upstream and in this contract, as wages-and-scheduled-expenses only; widening its meaning to absorb an unrelated income source would be the same "one system, one concern" violation this kind's ordering already avoids everywhere else (`employment` doesn't absorb `education`'s cashflow either). **Post business cashflow inside `finance_reconcile`** (which already runs after `housing`) — rejected because business revenue needs to be spendable against *this week's* rent the same way wages are (`finance_income`'s own stated reason for running before `housing`), and `finance_reconcile` runs after `housing`, too late for that money to help pay it.
Reversibility: moderate. New system name and new position in a tested, normative order — reversible before any campaign ships with businesses, costlier after (a shipped campaign's replay would depend on the exact post week for revenue/expenses relative to housing).

### 2026-08-31 — W101 contract gate 4: a rival resolves through `ResolverTable` like the player, one `{ kind: "agent" }` RNG stream per rival, in `WorldState.agents` order
Context: W101.5 requires that "the player and rivals use the same `ActorState` mechanics and the same action resolvers," proven by a fixture giving both actors identical state and the same chosen action. W101.6 requires that a rival's choices depend only on its allowed public view and "its stable RNG stream," immune to registry-construction reordering or an enabled recording emitter. §8 already declares the `{ kind: "agent"; agentId; seq }` `StreamId` variant precisely for this, unused until now — "keyed by agentId... not the action seq... an agent's randomness [must not] depend on how many actions preceded it."
Chosen: `end_week` resolves the player's plan first, then walks `WorldState.agents` in array order (the campaign's own `rivals` order, fixed at `initialState`, never re-sorted). For each, `selectActions(publicView, agent)` may draw from `deriveStream({ kind: "agent", agentId: agent.id, seq: agent.rngSeq })`, incrementing the new `AgentState.rngSeq` field once per draw; the returned `GameAction[]` resolves through the same `ResolverTable` (§5.1) already used for the player, actor-addressed by `agent.id`.
Rejected: **Resolve all actors' actions interleaved by some priority order** (e.g. highest-need-first) rather than player-then-agents-in-array-order — rejected as unnecessary complexity: nothing in W101's criteria asks for interleaving, and a fixed, content-order sequence is simpler to reason about and to test than a runtime-computed one, with no stated need the simpler rule doesn't already meet. **A single shared RNG stream for all rivals** — rejected directly against §8's own stated purpose for the `agent` variant and against W101.6's own wording ("its stable RNG stream," singular per rival).
Reversibility: moderate — additive (`rngSeq` is a new `AgentState` field, meaningless until a rival exists) but the resolution-order rule itself would be a breaking change to any campaign that has since shipped with rivals.

### 2026-09-01 — W102 contract gate 1: cross-game kind data is one opaque `PlayerProfile` slice per kind, folded from audit records
Context: `design/30-slices.md` W102 is contract-gated on "a fresh `/contract` pass defining both public shapes before implementation," and W102.1 asks for "a versioned, JSON-serializable, kind-owned data boundary whose ownership, unknown-kind behaviour, size/version validation, and migration order are stated in the canonical contract without importing a simulation type into core." The standing open item ([issue #268](https://github.com/The-Running-Dev/SubZeroDev.GameEngine/issues/268)) is `ChainScope`'s `"profile"` value having nowhere to persist: `PlayerProfile` was `{ formatVersion, profileId, achievements, terminals }`, all three payloads core-declared, and a kind-declared shape had no home.
Chosen: `PlayerProfile.kindData: readonly KindProfileRecord[]` — at most one `{ kindId, dataVersion, data }` per kind, `kindId`-ordered, `data` typed `unknown` exactly as `GameState.kindState` is (§2), with `formatVersion` moving 2 → 3 by a total migration. The kind half is one optional seam member, `Kind.profileData: KindProfileData` (`{ version, fold, migrate? }`), grouped rather than three correlated optionals so a kind cannot declare a version without a fold. `fold(current, campaign, changes)` is a pure, idempotent reduction over the same `AdvanceResult.changes` the achievement mirror already switches on, run by the store on the same profile-keyed write; a canonically-equal result means no write, which is what makes idempotence observable rather than asserted. Size is capped at 65 536 canonical bytes per record and checked on the write, so a tightened limit can never make a stored profile unloadable. Unknown `kindId`s round-trip byte-identically and are never handed to a kind — which is why `KindProfileRecord.kindId` is `string` and not `KindId`, since a closed union would condemn the whole profile as `profile_corrupt`. Migration order is `formatVersion` (adapter-owned, needs no registry) then `dataVersion` (core-owned, needs the registry), the same envelope-before-payload reasoning §10.2 already uses for kind-before-campaign. Two new base reason codes, `profile_kind_data_unreadable` and `profile_kind_data_rejected`, carry the two degradations.
Rejected: **A second store beside `ProfileStore`** — symmetrical with how the profile itself was added beside the session store, and rejected because it is a new host port for data with the same lifetime, the same key, the same degradation rules and the same write moment as the two mirrors already there; a host would implement two adapters that must be written together or corrupt each other. **A core-declared union of every kind's profile shape** — rejected outright; it points the dependency arrow backwards, which is the one thing `kindState: unknown` exists to prevent. **Three flat optional seam members (`profileDataVersion?`, `foldProfileData?`, `migrateProfileData?`)** — matches `terminalCount?`/`migrateState?`'s existing flat style, and rejected because the three are not independent: a version with no fold is a half-wired kind the compiler would accept. **Reusing `profile_corrupt` for both new degradations** — rejected; it says "starting with an empty one," which is false when achievements and terminals survived and only one kind's slice did not, and a misleading message to a player is worse than a code nobody switches on.
Reversibility: expensive. `formatVersion: 3` is durable player data and a literal type; `Kind.profileData` is a seam member. Both are additive, neither is quietly withdrawable. The 65 536-byte cap is the cheap part — raising it is compatible, lowering it is not, which is why it is checked on the write.

### 2026-09-01 — W102 contract gate 2: a profile slice reaches a new session through `NewGameConfig`, not through `KindContext`
Context: W102.2 requires a profile-scoped chain to be "visible to a new session for the same profile." §7.1's standing rule is that nothing in resolution reads a profile, and §7's is that `profileId` never reaches `NewGameConfig` or `GameState`. Something still has to carry the resolved value from the store to `Kind.initialState`, and the obvious candidate — `KindContext` — is the one `90-decisions.md` already rejected for the projection half of #278, on the grounds that a projection varying by profile is no longer reproducible from state.
Chosen: `NewGameConfig.kindProfileData?: unknown`, resolved and migrated by the store before `createSession` calls the pure engine, and delivered to `initialState(campaign, ctx, profileData?)` as an optional third argument. No identity travels and no ambient read happens during resolution: by the time `advance` runs, whatever the kind kept is ordinary `kindState`. Only `createSession` seeds — `resumeSession`, `loadGame` and `branchSession` explicitly do not, since re-reading a profile that has moved on would make a resumed session differ from the one that was saved.
Rejected: **`KindContext.profileData`** — rejected for the same reason its projection sibling was, one step earlier: `ReplayFixture.config` is a `NewGameConfig` (07 §2), so a value carried on the context is *not* captured with the fixture, and a captured session that started from a profile would silently stop being reproducible while the oracle kept passing. That is a worse failure than an unreproducible projection, because the harness would be asserting something weaker than it claims and nothing would say so. **A fourth `Engine.createGame` parameter, off `NewGameConfig`** — same defect for the same reason; the fixture records the config, not the call. **Not seeding at all, and exposing profile chains only through a projection field** — rejected against W102.2's own wording, and it would put cross-session state on the projection boundary, which is the erosion §9 exists to prevent.
Reversibility: moderate. `kindProfileData` is optional and absent from every existing fixture, so no committed replay outcome moves; `initialState`'s third parameter is optional and source-compatible with every current implementation. Withdrawing it later would strand any profile data already written, which is the durable half.

### 2026-09-01 — W102 contract gate 3: profile chains record `furthestStep` only; cumulative cross-game weeks stays unresolved
Context: `20-contract.md` 10 §2.2 has said since W32 that a `"profile"`-scoped chain "advances on cumulative weeks played across every game under one profile." W102.3 requires that reapplying an already-recorded transition creates no duplicate "including after save/load and process restart," and gate 1 above caps a slice at 65 536 canonical bytes. Those two constraints and the cumulative-weeks sentence cannot all hold.
Chosen: `SimulationProfileData.chains: { campaignId, chainId, furthestStep }[]`, keyed and ordered by `(campaignId, chainId)`, where `furthestStep` is a maximum. A maximum is idempotent under a refold by construction, so the store's canonical-equality check sees no change and writes nothing — which is exactly what W102.3 asks for, and it needs no dedup bookkeeping to get it. The cumulative-weeks half is stated as unresolved in `20-contract.md`'s new `## Unresolved` section and carried in this register, with §2.2 rewritten to say the phrase survives as intent and not as contract.
Rejected: **A `weeksPlayed` sum on the chain record** — the literal reading of the existing sentence, and not idempotent: refolding the same action's changes adds twice. **Per-`gameId` week records summed at read time** — idempotent (each is a maximum) and correct under concurrency, and rejected because it is unbounded: a profile accumulates one record per game ever played, which is precisely the unbounded accumulator the size cap exists to refuse. **A settle-on-game-change counter** (`weeksSettled + lastGameId + lastGameWeeks`) — bounded and idempotent, and rejected because it races two live sessions sharing one `profileId`, which is not a hypothetical: §7's second lock domain exists because that is a normal thing for a host to allow. Each alternative gives up exactly one of the three properties, and choosing which to give up is a design decision this document is not the place to take.
Reversibility: cheap while it stays unresolved — `SimulationProfileData` is `dataVersion: 1` with a declared migration seam, so adding a weeks field later is a version bump against a mechanism built to take one.

### 2026-09-01 — W102 contract gate 4: a published simulation migration is four declarative ops over a derived id-domain union, and the portable format version does not move
Context: `design/30-slices.md`'s prerequisite P7 — "a published simulation campaign cannot carry a save migration." `PortableMigration` is `nodeMap`/`endingMap` on the story-graph arm of `PortableCampaignBody`, which `src/engine/src/portable/format.ts` documents as a structural fact. It is not one about the format; it was one about how many kinds had a declarative migration written. W102.4 asks for "published-id remaps, required defaults, removals, and validation," as "data or engine-owned declarations, never host callbacks," in deterministic order.
Chosen: `SimulationMigration { fromVersion, steps }` on the simulation arm, with `SimulationMigrationStep` a closed four-member union — `remap`, `remove`, `default`, `require` — applied in array order. `SimulationIdDomain` is **not enumerated in the document**: it is a closed union with one member per `SimulationCampaign` collection whose ids `SimulationKindState` actually references, declared in one engine-owned module together with the reference-site table that gives each member its state paths, with a Tier 1 check rejecting a migration naming an uncovered domain. `PortableCampaign.formatVersion` stays `2`. Failure reuses `migration_failed` and `save_requires_migration` unchanged. §10.2 gains the rule the four ops make structural: a migration may re-address published ids and drop or default what no longer resolves, and may never invent play.
Rejected: **Enumerating `SimulationIdDomain`'s members here** — rejected on this file's own evidence: §3's system list and §10's code table have each already outlived the units that changed them, and both failures are recorded in this document. A derived union with a gate is the shape that cannot drift. **A path expression per step** (`"player.employment.jobId"`) — rejected as a mini-DSL: it would need its own grammar, its own validator, and its own escaping rules, to express what a domain plus an engine-owned reference-site table expresses with none of them, and it would let a published document address state the kind never meant to expose. **Bumping `PortableCampaign.formatVersion` to 3** — rejected because the silent-loss case cannot happen: a `2` reader ignores the new member and attaches no `migrateState`, which matters only when `campaignVersion` moved, and that is exactly when `resolveSaveEnvelope` already rejects with `save_requires_migration`. The old reader fails loudly at the one moment the dropped field mattered, and `formatVersion` is a literal type, so a bump would break every reader including the ones that were fine.
Reversibility: expensive once a campaign publishes with one. The step vocabulary becomes a wire format the moment a portable document carries it, and a published document is fetched by hosts this repository does not control. Adding a fifth op later is additive and cheap; changing what an existing one means is not.

### 2026-09-01 — W102 contract gate 5: `ChainScope` is authored on a new `EventChainDefinition`, carrying id and scope and nothing else
Context: §2.2's seeding rule needs `scope` to be reliably known for every chain that can fire, and `EventChainState.scope` had no source — the only chain vocabulary a campaign could write was `EventDefinition.chainId`/`chainStep` (§7.6), which names a chain without saying what kind of thing it is. W102.2 says "a simulation chain **declared** with profile scope."
Chosen: `SimulationCampaign.eventChains?: readonly EventChainDefinition[]` (§7.13), each `{ id, scope, labelKey? }`. Absent means `[]`, which is every campaign shipped before this field. Tier 1 rejects duplicate chain ids (`duplicate_id`) and an `EventDefinition.chainId` naming no declared chain (`dangling_reference`); the second is what makes the seeding rule safe. A declared chain nothing references is `unreachable_content` at Tier 2, like any other unreferenced definition.
Rejected: **Carrying the ordered step list on the definition** — the obvious fuller shape, and rejected as the envelope-duplication defect one level down: membership and order are already fully determined by the events that declare `chainId`/`chainStep`, so a second copy can disagree with the first, silently. `CLAUDE.md` records five instances of exactly this pattern and no gate caught any of them. **Inferring scope from a naming convention on `chainId`** — rejected; it makes a content-model decision unreviewable and unvalidatable, and a typo would change a chain's lifetime. **Defaulting an undeclared `chainId` to `"game"` scope instead of failing Tier 1** — rejected because it makes the failure mode "a profile chain quietly never persists," which is the class of bug that survives a whole release.
Reversibility: cheap. Optional and additive; no shipped campaign declares one, and `chainStates` stays `[]` for all of them.

### 2026-09-05 — `design/state-index.md` is deliberately absent; this repository mirrors work but does not project it
Context: `/track`'s final step runs `tools/Update-DesignProjection.ps1` to regenerate `design/state-index.md`'s `outstanding` region from the `WorkRef` records `Update-WorkMirror.ps1` just wrote, so mirror and projection land in one commit. The file has never existed here — `git log -- design/state-index.md` is empty — so every run refuses all six regions with `DocumentMissing` and exits 1, and `design/20-contract.md` likewise has no `invariants` region for the seventh. The mirror half is real and maintained: `design/state/work/` carries a record per open issue.
...
Context: `/contract` was invoked for "the invariants region", which resolves two ways. `tools/Update-DesignProjection.ps1:363` targets an `invariants` marked region in `design/20-contract.md`, rendering `Invariant` records from `design/state/`; the 2026-09-05 entry above records that region's absence as intended and says writing it "belongs to `/design` or `/contract` if it is wanted." Separately, `/contract` calls § Invariants "the highest-value section in the document," and this repository's copy was four lines of pure delegation: it named six cross-cutting families — determinism, envelope ownership, projection, migration, validation, identifiers — and stated none of them. The repository's only assertable invariants were the 19 in two section-scoped `#### Invariants` blocks (§7.1's P1–P8, §7.4's L1–L3/D1–D3/B1–B3/A1–A2), both added by recent slices. Every one of the six named families had zero.
