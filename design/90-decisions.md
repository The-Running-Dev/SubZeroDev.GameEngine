...
### 2026-08-04 — Kit upgrade to `92abff1`: adopted the broader tracker/git authorization carve-out
Context: Upgrading the agent kit from `9b8313c` to `92abff1` ("Automate issue/checkbox writes, and give `/slice` its own branch and draft PR"). This repository's `slice.md`/`pr.md`/`track.md`/`kit-help.md` and `CLAUDE.md` still carried the fully human-gated policy matching the kit's *previous* default (`/slice` never touches git; ticking a `Done when` box, closing an issue, and creating a milestone or project all wait on the user), not a deliberate override of it — the wording was a direct carry-over from the `9b8313c` install, before this expansion existed upstream.
Chosen: Adopt the new policy in full. `/slice` now branches (`slice/W<n>`), commits, pushes, and opens its PR as a draft after tests pass, ticking the `Done when` boxes it confirms in the same run; `/pr` adopts that draft, writes the real description, and asks before marking it ready for review; `/track` closes an issue once every box is ticked and creates a milestone or project directly instead of proposing one first. Deleting an issue, milestone, or project, and resolving or replying to a review thread, stay outside the carve-out exactly as before. Also added the session-boundary banner requirement (`CLAUDE.md`, *Session boundaries*) that arrived in the same kit commit — an unrelated, uncontroversial procedural addition bundled with the policy change upstream.
Rejected: **Keep the conservative, fully human-gated policy** — smaller diff and no new autonomy granted, but leaves this repository permanently behind the kit's current default and re-litigates a question the maintainer answered explicitly for this install (adopt). **Adopt the git/PR half but keep manual ticking, or vice versa** — rejected; the two halves are one coherent policy upstream (the tick and the report are "the same claim now, not two," per the kit's `slice.md`), and splitting them would leave `/track`'s auto-close relying on a signal (`/slice`'s tick) this repository would not actually be producing.
Reversibility: cheap — each file's change is independently revertible; nothing here is destructive or already exercised against a real issue or PR.
