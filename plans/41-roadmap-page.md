...
and how a question about *Jones in the Fast Lane* became forty-three merged work units. The
...
> **Counts corrected 2026-08-02, against `main`.** The first draft said forty-one throughout.
> `TODO.md` carries **42** completed unit headings — W0–W40, *including* `W3a` — and W41 has
> since merged and published, so the true figure is **43**. Forty-one is only reachable by
> counting W0–W40 and silently dropping `W3a`, which is a real unit with its own PR (#34).
> The headline is a number joke, so the number has to survive someone checking it.
...
- Hero: **“How a Quick Question Became 43 Work Units.”**
...
- Eyebrow: `THE ROADMAP`
- H1: `How a Quick Question Became 43 Work Units.`
- One plain-language deck.
- A compact, verified fact strip:
  - `43` merged work units
  - `2` implemented kinds
...
**Derive `43` rather than typing it.** It is the count of `### [x] W…` headings in `TODO.md`
(42, W0–W40 including `W3a`) plus W41, whose entry is a `[~]` bullet rather than a heading
because W41 predates its own completion. Write the test against that derivation, not against
the literal — a hard-coded number is what produced the wrong one in the first draft, and the
next unit to merge makes it wrong again.

### 3.3 “Previously, on the game engine” — delivered history

Do not render W0–W41 as forty-three equal cards. That is technically complete and editorially
...
- a visible status label — `SHIPPED` for all six, W41 included, since its package published;
...
| The engine gets a front door | W41 | Shipped | A supported package boundary, a consumer test that installs the real tarball, a release workflow — and, as of `v0.4.0`, a published package another project can install. | [`db9c62a`](https://github.com/The-Running-Dev/SubZeroDev.GameEngine/commit/db9c62aec509ed083179a73ae2ec49b1b53d3d26) |
...
> **This section was written when W41 was the checkpoint, and it no longer is.** The package
> published as `@the-running-dev/game-engine@0.4.0` on 2026-08-02, which was the last thing
> W41 was waiting on. The honest checkpoint moved forward by one unit; the copy below is
> rewritten rather than patched, because "not yet published" was its whole spine.

Status copy:

- W41 is complete: the boundary is merged, the clean-consumer gate passes, and `v0.4.0` is
  published and installable.
- The third kind's contract is being written now — W42, the first of three specification
  units, on a branch and **not merged**.
- Nothing of the world-graph kind runs yet. A contract is a description, not a program.
- Do not display that branch as delivered work. If it has merged by implementation time, move
  it into the shipped chapters and make W43 the checkpoint.
...
Suggested heading: **“The Door Is Open. Somebody Has to Describe the Resort.”**

> **A caveat worth carrying, not burying.** The package published **public**, while
> `plans/39`, `plans/40` and `TODO.md` all describe it as private. Whether that is corrected
> in the documents or in the package's visibility is unresolved
> ([`OPEN-QUESTIONS.md`](../docs/docs/engine/OPEN-QUESTIONS.md)). Until it is, the page should
> say "published" without saying "private" — both repositories are public, so the word adds a
> claim the page cannot stand behind.
...
**This phase has already caught this plan out once.** Between drafting and the first review,
the package published and the unit count was found wrong; both are corrected above. Assume it
will happen again — the gap between writing copy and shipping it is where a roadmap rots.

- Pull `main` and inspect `TODO.md`, `plans/39`, `plans/40`, tags, and the package registry state.
- **Recount from the artifact, not from this plan.** Count `### [x] W…` headings in `TODO.md`,
  add any `[~]` unit whose work has since merged, and compare with the hero figure above. Do
  not trust a number written in prose, including this one.
- Check the registry directly for published versions rather than inferring from a workflow run
  or a tag: `gh api "/user/packages/npm/game-engine/versions"`.
...
- Six shipped chapters render in chronological order, W41 among them.
- The work-unit count in the hero is derived from `TODO.md`, not a literal, and matches it.
- W42–W49 are grouped into three future acts and never labelled shipped.
- The checkpoint names unmerged work as unmerged, and no future act is described as running.
...
- [ ] W0–W41 are represented as six shipped chapters, with an accurate checkpoint after them.
- [ ] The hero count is derived from `TODO.md` and recomputed at implementation time.
...
- No claim about package publication that the registry does not prove. It now proves `0.4.0`
  exists; it does not prove the package is private, and the page should not say so.
...
| Story shape | Six shipped chapters → W42 in progress → three next acts → later horizons |
...
| Current truth | W41 complete and published at `v0.4.0`; W42 in progress on a branch, unmerged |
