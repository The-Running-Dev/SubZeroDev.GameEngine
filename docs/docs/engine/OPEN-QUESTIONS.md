...
- **`world-graph` W42 content follow-up.** The runtime-state contract is authoritative: entrances
  are derived from W43 footprint-relative authored offsets and building rotation, and
  `GuestOpinions` stores seven fields. Any future content-definition changes must preserve that
  runtime-state boundary. **Revisit when** W43 content definitions are merged.
