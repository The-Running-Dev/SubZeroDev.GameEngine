...
- **W63.7/W63.8 remain partially unverified.** The browser harness provides useful infrastructure,
  but the PlayApp specs do not yet apply the font, line-height, hit-area, and gap assertions to
  real rendered controls, nor do they cover ready-state snapshots, keyboard-only use, 200% zoom,
  long text, missing assets, or forced-colors behavior in the application. Revisit when those
  scenarios are driven against the actual UI.
