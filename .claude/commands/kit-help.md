...
2. **`/slice W42`**, or bare **`/slice`** for the lowest-numbered slice that is neither closed nor fully ticked and whose dependencies are done. Criteria stated by id, failing tests first, implemented against the contract, ending with the ids it believes are met.
3. **You tick the boxes.** Deliberately not the agent's to do — "W42.1 met" and a ticked box are different claims by different parties.
