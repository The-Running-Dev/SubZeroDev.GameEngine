...
A slice is **done** when its issue is closed, or when every box under `Done when` is ticked. Ticking is the user's confirmation that a criterion is genuinely met (`CLAUDE.md`, *The Agent Kit — Canonical Workflow*), which makes the tracker the only evidence of doneness this kit recognises. **Do not infer it from the working tree, the commit log, or code that appears to already exist** — that is equally what an abandoned attempt looks like.
...
1. State the slice's acceptance criteria back as a checklist, **by id** — `W42.1`, `W42.2`. One line each. Nothing else.
