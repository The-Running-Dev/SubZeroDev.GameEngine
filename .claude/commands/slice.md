---
description: Implement one slice. Usage - /slice W42, or /slice for the next one
...
This repository's slice ids use the established `W` prefix. Read the complete marked delivery
ledger; do not infer status from the generated `docs/docs/engine/TODO.md` copy.
...
1. **Branch.** `git status --short` must be clean and on the default branch before you touch anything; uncommitted work that is not this slice's is not yours to stash or discard (`CLAUDE.md`, *Safe Start*) — stop and say so instead. Create and check out `slice/W<n>` from the default branch's latest. **Refuse to implement on the default branch** — `/pr` refuses to open a PR from it, and the branch is one command away.
2. State the slice's acceptance criteria back as a checklist, **by id** — `W42.1`, `W42.2`. One line each. Nothing else.
...
6. **Commit, then push.** Stage by named path — never `git add -A`, `git add .`, or a bare directory (`CLAUDE.md`, *Git and Pull Requests*).
7. **Open the pull request as a draft.** Carved out of the authorization rule the same as pushing the branch (`CLAUDE.md`, *Git and Pull Requests*) — a draft requests no review and blocks no merge, so it needs no sign-off. Title it from the slice name; the body can be minimal, since `/pr` writes the real description once `/verify` has run in this same session. Check for an existing open PR on this branch first and do not open a second one.
8. **Tick the `Done when` boxes** on the matching issue for every id this run confirms met. Carved out the same way (`CLAUDE.md`, *Tracking work*) — the report in step 9 and the tick are the same claim now, not two.
